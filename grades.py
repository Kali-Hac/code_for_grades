...
def get_info_2():
	...
	opener=urllib2.build_opener(handle)
	#实际上教务网没有cookie验证，为了规范还是写上去
	postdata=urllib.urlencode(allrequest_data.data['2017'])
	req = urllib2.Request(allrequest_data.url2['2017'],postdata,headers=allrequest_data.headers['2017'])
	response = opener.open(req)
	html=response.read()
	html=html.decode("gb2312")
	outputFp = codecs.open("d:\\0test.txt", 'w', 'gbk')
...
def get_info_3():
	...
	req = urllib2.Request(allrequest_data.url2['专业选修'],postdata,headers=allrequest_data.headers['专业选修'])
	response = opener.open(req)
	html=response.read()
	html=html.decode("gb2312")
	outputFp = codecs.open("d:\\0test.txt", 'w', 'gbk')
